...
void ADC_init(ADC_CFG1_DLPC power, ADC_CFG1_ADIV div, ADC_CFG1_ADLSMP sample, ADC_CFG1_MODE mode, ADC_CFG1_ADICLK clk)
{
	...
	ADC0->CFG1 |= ADC_CFG1_MODE(2);//16 bits ADC
	ADC0->SC1[0] |=ADC_SC1_ADCH(31);//Disable module
	ADC0->SC3 = ADC_SC3_AVGE_MASK | ADC_SC3_AVGS(3);//average of multiple times
...
ufloat32 setMeasuredValue(){
	...
	for(int i =0;i<32;i++){
		...
		delay(delayValue);
			...
			return (ufloat32) (a/32);
}

ufloat32 convertRead(){
	return ADC_Values()*(0.080566);
}
